                    ...
                    for image_y in range(-N_images, N_images+1):
                        for image_z in range(-N_images, N_images+1):
                            ...
                            r_ij = r_i - r_j - np.array([image_x*simcell[0],
                                                         image_y*simcell[1],
                                                         image_z*simcell[2]])
                            dist_ij = sqrt(r_ij[0]*r_ij[0] + r_ij[1]*r_ij[1] + r_ij[2]*r_ij[2])
                            if dist_ij > dist_cut_off:
                                continue
                                ...
                                if orb_i == orb_j:
                                    element += on_site_energy_table(
                                          self.species_dict[self.atom_id_dict[i]], orb_i)*bloch_phase_factor(k_point, r_ij)
                                else:
                                    continue
                            else:
                                element += slater_koster_table(orb_i, orb_j, r_ij[0]/dist_ij,
                                                              r_ij[1]/dist_ij, r_ij[2]/dist_ij,
                                                              dist_ij)*bloch_phase_factor(k_point, r_ij)
                self.H[i, j] = element
